...
# t.me/freefilm_bot_tt_bot
TOKEN = '6364962220:AAFpopszLdgU_PTQUoCG2JBqxoMLAcFiJ0A'
...
def main(message):
    ...
    ls_dict[message.chat.id].append(
        bot.send_message(message.chat.id,
                         "<b>Привет</b> 👋  \n В этом боте ты сможешь получить ссылку на сайт с бесплатными новыми фильмами в самом лучшем качестве без рекламы во время фильма \n Жми «продолжить», чтобы получить ссылку 🚀🤫",
...
def callback_worker(call):
    ...
    if call.data == "end":
        if not check_valide_chats(call.from_user.id):
            ...
            strms = "<b>Готово!</b> ✅ \n \n"
            strms += "Теперь ты можешь получить ссылку на сайт с бесплатными фильмами 🍿 \n \n"
            # strms += "подписки на спонсоров, приносим извинения за ожидание ❤️ \n \n"
            strms += "‼️<b>Перед просмотром фильмов необходимо зарегистрироваться на сайте </b> ‼️\n \n"
            strms += "🚫Во время ожидания нельзя отписываться от каналов \n"
            strms += "спонсоров, иначе отправка не произойдет. \n"
            markup = types.InlineKeyboardMarkup()
            btn_sp = types.InlineKeyboardButton('👉 СМОТРЕТЬ ФИЛЬМЫ 👈', callback_data='btn', url='https://1wgxuy.top/tv')
            markup.add(btn_sp)
            bot.send_message(call.message.chat.id, strms, parse_mode='html', reply_markup=markup)
